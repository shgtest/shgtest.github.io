								...
								if (p.code.stype == 3) { // feed
									...
									if (this.is_mobile()) {
										if (p.b == 'ids') { // ids click first
											...
											code += '<a target="_blank" href="' + ids_curl + '" onclick="iis.m_click('+"'"+ iis_curl +"'"+');"><img src="' + p.code.aurl + '" style="width:100%;border:none;"/></a>'+"\n";
										} else {
											code += '<div style="width:30px; height:10px; border:1px solid #bababa; border-radius:2px; font-size:10px; text-align:center; position:absolute; right:4px; bottom:4px; color:#bababa; line-height:9px; z-index: 999;">广告</div>'+"\n";
											code += '<a target="_blank" href="' + p.code.curl + '"><img src="' + p.code.aurl + '" style="width:100%;border:none;"/></a>'+"\n";
										}
									} else {
										code += '<div id="iis_rtbs_' + p.ap + '" style="position:relative;margin:0;padding:0;width:'+p.w+'px;height:'+p.h+'px;">'+"\n";
										code += '<script type="text/javascript">'+"\n";
										code += 'var a = "'+p.code.curl+'";'+"\n";
										code += 'var ad = "'+p.code.aurl+'";'+"\n";
										code += 'var w = "'+p.w+'";var h = "'+p.h+'";var isActive = 0;'+"\n";
										code += '</script>'+"\n";
										code += '<script type="text/javascript" src="http://h0.ifengimg.com/ifeng/sources/yingguang-20140307.js"></script>'+"\n";
										 // logo
